 ...
 *
 */
public class CompanyURLProcess extends FujianProcessHandlePrepare{
	public CompanyURLProcess(){
		this.isDownloadCodeProcess = false;
		this.isStorageProcess = false;
		setUniqueWebUri("/creditpub/search/ent_info_list");
			...
			if(maxNumber!=null&&maxNumber.matches("\\d*")){
				...
				if(currentPage<max){
					for(int x=currentPage+1;x<=max;x++){
						...
						String url = builderURL("/creditpub/search/ent_info_list?"+urlTail(), task.getSite());
