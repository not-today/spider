 ...
 *
 */
public class CompanyURLProcess extends AdministratorProcessHandlePrepare{
	public CompanyURLProcess(){
		this.isDownloadCodeProcess = false;
		this.isStorageProcess = false;
		setUniqueWebUri("http://gsxt.saic.gov.cn/zjgs/search/ent_info_list");
			...
			if(maxNumber!=null&&maxNumber.matches("\\d*")){
				...
				if(currentPage<max){
					for(int x=currentPage+1;x<=max;x++){
						...
						String url = builderURL("/zjgs/search/ent_info_list?"+urlTail(), task.getSite());
