 ...
 *
 */
public class ContexDJProcess extends HaiNanProcessHandlePrepare {
	...
	public void analyticalProcess(Page page,PageProcessor task) {
		...
		if(Method.GET.equals(page.getRequest().getMethod())){
			...
			if(match1(isNext)){
				...
				String url = builderURL("/aiccips/GSpublicity/invInfoPage.html?"+urlTail()+"&"+Math.random(), task.getSite());
			...
			if(match1(trbg)){
				...
				String url = builderURL("/aiccips/GSpublicity/entChaPage?"+urlTail()+"&"+Math.random(), task.getSite());
				Request request = builderRequest(url, Method.POST, regNumber, entName, nvps);
				page.addTargetRequest(request);
			}
			...
			String url = builderURL("/aiccips/GSpublicity/GSpublicityList.html?service=entCheckInfo&"+urlTail(), task.getSite());
			Request request = builderRequest(url, Method.POST, regNumber, entName, nvps);
			page.addTargetRequest(request);
		}else{
			//主要人员
			...
			if(match1(tr)){
				...
				NameValuePair[] nvps = {
					...
					String url = builderURL("/aiccips/GSpublicity/vipInfoPage?"+urlTail()+"&"+Math.random(), task.getSite());
					Request request = builderRequest(url, Method.POST, regNumber, entName, nvps);
					page.addTargetRequest(request);
			}
			//分支机构
