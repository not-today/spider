...
public class GDImgageHandle {
	...
	private static void run() throws IOException {
		...
		File testDataDir = new File("C:\\Users\\tst\\Desktop\\gd-code");
		for (File file : testDataDir.listFiles()) {
			...
			// 清除背景颜色
			bufferedImage = cleanBg(bufferedImage,testDataDir.getAbsolutePath() + "/01", file.getName());
			// 根据平滑度降噪
			bufferedImage = filterBySmooth(bufferedImage,testDataDir.getAbsolutePath() + "/02", file.getName());
			// 更具颜色切图
	...
	public static BufferedImage cleanBg(BufferedImage bufferedImage,String dir, String fileName) throws IOException {
		...
		int avg_count = (sum-max_count)/(num-1);//求的除去背景色外的像素点出现的次数的平均数
